...
이유: 이 코드가 없으면 국수처럼 아주 길쭉한 방이 생길 수 있습니다. 가로가 세로보다 1.5배 길면 무조건 세로로 잘라서(splitHorizontal = false) 정사각형에 가깝게 유지하도록 강제하는 장치입니다.
        ...
        if (splitHorizontal)
        {
            node.left = new Node(new RectInt(node.rect.x, node.rect.y, node.rect.width, splitPos), node);
            node.right = new Node(new RectInt(node.rect.x, node.rect.y + splitPos, node.rect.width, node.rect.height - splitPos), node);
        }
        else
        {
            node.left = new Node(new RectInt(node.rect.x, node.rect.y, splitPos, node.rect.height), node);
            node.right = new Node(new RectInt(node.rect.x + splitPos, node.rect.y, node.rect.width - splitPos, node.rect.height), node);
        }
    ...
    void GenerateContents(Node node)//4
    {
        if (node.IsLeaf())
        {
            // 방 크기 결정 및 그리기
    ...
    void DrawCorridor(Node left, Node right)//5
    {
        // 각 구역의 중심점 계산
